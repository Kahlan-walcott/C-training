...
We have a program that asks and checks for a pin. It uses a while loop, which will be the first type of loop we’ll learn
about, to ask the user to type and enter a pin over and over again until the correct one is entered.
...
// there is a terminal on the side of the code
/* Compile and execute in the terminal using

gcc interacting-with-a-loop.c
./a.out
